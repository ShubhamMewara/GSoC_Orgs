...
export const BentoGridItem = ({
  ...
  return (
    ...
    onClick={()=>{window.open(`/organization/${title}`)}}
      className={cn(
        "row-span-1 rounded-xl group/bento hover:shadow-xl transition duration-200 shadow-input dark:shadow-none p-5 dark:bg-gray-700 bg-white border border-transparent justify-around flex flex-col space-y-4 cursor-pointer", 
        className
      )}
    >
      <img className='flex justify-center mx-auto h-1/3  group-hover/bento:translate-x-1 transition duration-200' src={img} alt="img" />
      <div className="group-hover/bento:translate-x-1 transition duration-200">
        ...
        <div className="mt-1 py-1">
          {technologies?.map((technologie:string,i) => (
